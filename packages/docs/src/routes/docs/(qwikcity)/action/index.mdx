...
Actions can be declared using `routeAction$()` or `globalAction$()` exported from `@khulnasoft.com/qwik-city`.
...
The best way to call an action is using the `<Form/>` component exported in `@khulnasoft.com/qwik-city`.
...
import { component$, type Signal, useSignal } from '@khulnasoft.com/qwik';
...
Actions can be declared using the `routeAction$()` or `globalAction$()` exported from `@khulnasoft.com/qwik-city`, the only difference between the two is that `routeAction$()` is scoped to a route, while `globalAction$()` is globally available across the whole app.
...
import { globalAction$ } from '@khulnasoft.com/qwik-city';
