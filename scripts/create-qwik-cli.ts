...
async function updateBaseVersions(config: BuildConfig, version: string) {
  ...
  console.log(`   update devDependencies["@khulnasoft.com/qwik"] = "${semverQwik}"`);
  baseAppPkg.devDependencies['@khulnasoft.com/qwik'] = semverQwik;
...
async function updatePackageJson(config: BuildConfig, destDir: string) {
  ...
  if (pkgJson.devDependencies && pkgJson.devDependencies['@khulnasoft.com/qwik']) {
    pkgJson.devDependencies['@khulnasoft.com/qwik'] = qwikVersion;
  }
