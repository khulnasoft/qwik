...
0. Vitest is configured to run a setup function once **PRIOR TO ALL** tests. During the setup `@khulnasoft.com/qwik`, `@khulnasoft.com/qwik-city` and `eslint-plugin-qwik` packages will be packed with `pnpm pack` Those will be used at a step 2 for every test. Tarballs are located in `temp/tarballs` folder within this repo. It is assumed that packages are built before E2E is executed.
...
2. Uses packed `@khulnasoft.com/qwik`, `@khulnasoft.com/qwik-city` and `eslint-plugin-qwik` packages to update package.json file of the generated application with `file:path-to-package.tgz`.
